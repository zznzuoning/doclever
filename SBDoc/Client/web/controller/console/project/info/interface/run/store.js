...
module.exports = {
    ...
    actions: {
        run: function (context) {
            ...
            var baseQuery=query;
            if (method=="GET") {
                path = path + "?" + helper.jiamiParam(header,baseQuery,"get");
            }
            ...
            if (bUpload || context.getters.curParam.bodyInfo.type == 1) {
                ...
                body = helper.jiamiParam(header,query,"post");;
            ...
            return func.then(function (result) {
                let _time = result.data.data.timestamp;
                let _data = result.data.data.data;
                let oss = header.os || '';
                let keyss = helper.createKeyAes(_time, oss)
                let deData;
                if (JSON.stringify(_data) !== "{}") {
                    deData = JSON.parse(helper.decryptAes(_data, keyss))
                }
                let decryptAesReuslt = {
                    status:result.data.status,
                    message:result.data.message,
                    data:deData
                };
                result.data = decryptAesReuslt;
