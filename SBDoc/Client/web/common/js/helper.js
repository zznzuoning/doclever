...
helper.jiamiParam = function (header, data, method) {
    let result;
    let timestamp = new Date().getTime();
    let os = header.os || '';
    let keys = helper.createKeyAes(timestamp, os);
    let token = header.token || '';
    let identity = header.identity || 1;
    let channelid = header.channelid || '';
    let admintoken = header.admintoken || '';
    let regid = header.regid || '';
    let uidcode = header.uidcode || '';
    let _data = {
        paramdic: data,
        headerdic: {
    ...
    // AES加密参数
    let enData = helper.encryptAes(JSON.stringify(_data), keys)
    let params = {
        data: enData,
        timestamp: timestamp
    }
...
//加密
helper.encryptAes = function (word, keyStr) {
...
//解密
helper.decryptAes = function (word, keyStr) {
    var key = CryptoJS.enc.Utf8.parse(keyStr); //Latin1 w8m31+Yy/Nw6thPsMpO5fg==
...
// md5加密
helper.md5 = function (word) {
    let str = CryptoJS.MD5(word).toString();
    return str;
}

//生成key
helper.createKeyAes = function (timestamp, os) {
    // let appkey = CryptoJS.MD5('zhili$admin2020').toString();
    // let appSercet = CryptoJS.MD5('zhiliApi@@@@@2020fighting').toString();
    let appkey = '4EEBA2AFC8B572E898A02F3B67B68CD7';
    let appSercet = '82BD2C1FB1DAC43AD7D26634B3D55652'
    switch (os.toLowerCase()) {
        case "pc":
            appkey = '7099B488BD154A2D9CEEAD4C042DEEFC';
            break;
        case "ios":
            appkey = 'ED18174D8468136750B6CE63A8768A1B'
            break;
        case "android":
            appkey = '3054E13DC3A548176EDF4E44C8B7966D'
            break;
        case "miniplateform":
            appkey = 'FBD8B1DD8EF5B5B7F1C11BFC9A57F6E8'
            break;
        default:
            appkey = appkey;
            break;
    }
    let key = CryptoJS.MD5(`appkey=${appkey}&timestamp=${timestamp}&appsercet=${appSercet}`).toString().toUpperCase();
